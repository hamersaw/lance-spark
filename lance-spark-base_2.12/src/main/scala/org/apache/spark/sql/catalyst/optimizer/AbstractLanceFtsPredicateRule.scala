 ...
 * {@code lance_phrase}, {@code lance_multi_match}) in a Filter condition and pushes the FTS
...
abstract class AbstractLanceFtsPredicateRule extends Rule[LogicalPlan] {
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan.transformDown {
    case filter @ Filter(condition, rel: DataSourceV2Relation) =>
      ...
      if (ftsExprs.isEmpty) {
        ...
        val newOptionsMap = new java.util.HashMap[String, String](rel.options.asCaseSensitiveMap())
   ...
   * AnalysisException if any FTS expression is reachable via an OR node.
   * OR(FTS, FTS) gets a dedicated message naming lance_multi_match.
   */
  private def validateNoOrWithFts(expr: Expression): Unit = {
    expr match {
      case or: org.apache.spark.sql.catalyst.expressions.Or =>
  ...
  private def buildFullTextQuery(afe: ApplyFunctionExpression): FullTextQuery = {
    ...
    if (fnName == LanceMatchFunction.NAME) {
      ...
      throw new AnalysisException(s"Unsupported Lance FTS function: $fnName")
    }
  }
  ...
  private def buildMatchQueryWithOptions(
    ...
    for (part <- optsStr.split(",")) {
      ...
      if (trimmed.nonEmpty) {
        ...
        val key = trimmed.substring(0, eqIdx).trim.toLowerCase(java.util.Locale.ROOT)
        ...
        key match {
          case "fuzziness" =>
            ...
            fuzziness = Some(v)
          case "operator" =>
            operator = value.toUpperCase(java.util.Locale.ROOT) match {
  ...
  private def buildPhraseQuery(args: Seq[Expression]): FullTextQuery = {
    val column = extractColumn(args(0), "lance_phrase")
    val queryText = extractQueryText(args(1), "lance_phrase")
    val slop = if (args.size == 3) {
      args(2) match {
        case Literal(v: Int, IntegerType) =>
          if (v < 0) {
            throw new AnalysisException(
              s"lance_phrase: slop must be >= 0, got $v")
          }
          v
        case _ =>
          throw new AnalysisException(
            "lance_phrase: the slop argument must be a non-negative integer literal")
      }
    } else {
      0
    }
  ...
  // This means unknown keys reach parseMultiMatchOptions which raises AnalysisException early,
