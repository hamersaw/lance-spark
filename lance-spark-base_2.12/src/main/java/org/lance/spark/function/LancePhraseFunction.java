...
/**
 * Sentinel V2 function for FTS phrase search: {@code lance_phrase(col, query [, slop])}.
 *
 ...
 *
 * <p>The {@code slop} slot declares {@code IntegerType} so Spark enforces integer type at bind
 * time; it defaults to 0 (exact phrase match) when omitted.
 */
public final class LancePhraseFunction implements UnboundFunction {

  public static final String NAME = "lance_phrase";
  ...
  public BoundFunction bind(StructType inputType) {
    ...
    if (arity == 2) {
      ...
      return new Bound(
          new DataType[] {DataTypes.StringType, DataTypes.StringType, DataTypes.IntegerType});
    } else {
      throw new IllegalArgumentException(
          NAME + " expects 2 or 3 arguments (column, query [, slop]), got " + arity);
    }
  }

  @Override
  public String description() {
    return "FTS phrase search: lance_phrase(column, query [, slop]) -> BOOLEAN. "
        + "Requires an FTS index built with with_position=true. "
  ...
  private static final class Bound implements ScalarFunction<Boolean> {
    ...
    public Boolean produceResult(InternalRow input) {
      throw new UnsupportedOperationException(
          "LancePhraseFunction should never be called at row level — "
